...
use MuckiRestic\Library\RestoreInterface;

class AmazonS3 extends Configuration implements RestoreInterface
{
    public function createRestore(): ResultEntity
    {
        $this->setRepositoryPath('/');
        if(!$this->checkInputParametersByCommand(Commands::RESTORE_AMAZON_S3)) {
            throw new InvalidConfigurationException('Invalid configuration');
        }

        $process = $this->createProcess(Commands::RESTORE_AMAZON_S3);
        $process->run();
